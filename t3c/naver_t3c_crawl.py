...
def insert_db(posts):
    ...
    try:
        with MysqlConnector(connect_url) as db:
            query = (
                "insert into board(content, category_id, user_id) values(%s, %s, %s)"
            )
...
if response.status_code == 200:
    ...
    board_numbers = soup.select(
        "#main-area > div:nth-child(4) > table > tbody > tr > td.td_article > div.board-number"
    )
    titles = soup.select(
        "#main-area > div:nth-child(4) > table > tbody > tr > td.td_article > div.board-list > div > a.article"
    )
    board_dates = soup.select(
    ...
    for board_number, title, board_date, url, author in zip(
        board_numbers, titles, board_dates, urls, authors
    ):
